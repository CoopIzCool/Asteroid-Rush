...
public class Zone : MonoBehaviour
{
	...
	public void BuildZone(GameObject[] tilePrefabs, GameObject[] objectPrefabs)
	{
		switch (zoneType)
		{
			...
			case ZoneTypes.Tunnel:
				...
				break;
			case ZoneTypes.Maze:
				BuildMaze(tilePrefabs, objectPrefabs, (int)Random.Range(width * height * 0.3f, width * height * 0.5f));
				break;
		}
	}
	...
	private void BuildMaze(GameObject[] tilePrefabs, GameObject[] objectPrefabs, int numWalls)
	{
		...
		while (numWalls > 0)
		{
			...
			{
				wallWidth = 1;
				wallLength = Random.Range(3, height - 1);
			}
			else
			{
				wallWidth = Random.Range(3, width - 1);
				wallLength = 1;
			}

			upOrRight = !upOrRight;

			randomX = Random.Range(xPos + 1, xPos + width - wallWidth);
			randomZ = Random.Range(zPos + 1, zPos + height - wallLength);
